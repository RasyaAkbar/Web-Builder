...
const FunnelSteps = ({ funnel, funnelId, pages, subaccountId }: Props) => {
  const [clickedPage, setClickedPage] = useState<FunnelPage | undefined>( // page that is being clicked
    pages[0]
  );
  ...
  const onDragEnd = (dropResult: DropResult) => {
    ...
    //change state; locally swap around
    const newPageOrder = [...pagesState] // everything in pageState
      .toSpliced(source.index, 1)
      .toSpliced(destination.index, 0, pagesState[source.index])
      .map((page, idx) => {
        return { ...page, order: idx }; //change the order as the index
      });
  ...
  // Note: The funnel order can be used to identify the next page in the order of the funnel
  return (
    <AlertDialog>
