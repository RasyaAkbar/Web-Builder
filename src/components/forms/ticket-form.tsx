...
const TicketForm = ({ getNewTicket, laneId, subaccountId }: Props) => {
  ...
  const saveTimerRef = useRef<ReturnType<typeof setTimeout>>() //insane func!
        ...
        <Form {...form}>
          ...
          >
            ...
            <Select
              ...
              defaultValue={assignedTo} //default value if exists
            >
              <SelectTrigger>
                <SelectValue
                  placeholder={
              ...
              <SelectContent>
                {allTeamMembers.map((teamMember) => ( // map all team member to become select item
                  <SelectItem
                    key={teamMember.id}
                    value={teamMember.id}
                  >
                  ...
                  <CommandInput
                    ...
                    onChangeCapture={async (value) => {
                      ...
                      if (saveTimerRef.current)
                        clearTimeout(saveTimerRef.current) // 2. if they make a change in the search, appoint new timer
                      saveTimerRef.current = setTimeout(async () => {
                        ...
                        setContactList(response)
                        setSearch('')
                      }, 1000)// 1. set Timer everytime they make a search (1 sec interval before sending response) 
                    }}
                  />
